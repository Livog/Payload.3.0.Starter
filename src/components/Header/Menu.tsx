            ...
            <li key={item.title || index} className="group relative">
              <Link
                className="flex items-center gap-x-2 rounded-lg px-3 py-2 font-medium outline-none ring-blue-500/80 transition-all duration-300
                  hover:bg-black/5 focus-visible:ring-2 dark:hover:bg-white/10"
                href={item.path || '#'}>
                {item.title || 'No Title'}
              ...
              {item.subMenuItems && item.subMenuItems.length > 0 && (
                <ul
                  className={cn(
                    `invisible absolute left-0 top-full mt-2 grid w-fit origin-top scale-95 auto-rows-[minmax(0,1fr)]
                    grid-cols-[repeat(2,minmax(250px,1fr))] gap-1 rounded-xl border bg-zinc-100 p-1 opacity-0 transition-all duration-300
                    ease-in-out group-hover:visible group-hover:scale-100 group-hover:opacity-100 group-focus:visible
                    group-focus:scale-100 group-focus:opacity-100 group-[&:has(>a:focus-visible)]:visible
                    group-[&:has(>a:focus-visible)]:scale-100 group-[&:has(>a:focus-visible)]:opacity-100 group-[:not(:hover)]:delay-300 dark:border-zinc-700 dark:bg-zinc-800
                    md:origin-top-right`
                  )}>
                  {item.subMenuItems.map((subItem, subIndex) => (
                    <li key={subIndex}>
                      <Link
                        className="group/item flex w-full items-center gap-x-2 rounded-lg px-3 py-2 font-semibold transition-all duration-300
                          hover:bg-black/5 dark:hover:bg-black/25"
                        href={subItem.path || '#'}>
                        {subItem.icon && (
